...
class CVSMNet_SoftArgMin_3DUNetDisp(nn.Module):
    def __init__(self, maxdisp):
        super(CVSMNet_SoftArgMin_3DUNetDisp, self).__init__()
        self.maxdisp = maxdisp
        self.feature_extraction = feature_extraction()
        self.maxPool3d = nn.MaxPool3d(3, stride=(2,1,1),padding =(1,1,1))
        self.upsample3d = nn.Upsample(scale_factor=(2,1,1), mode='nearest')
    ...
    def estimate_disparity(self, cost):
        ...
        #down2
        down2 = self.maxPool3d(down1)
        # print("down2.shape=",down2.shape)
        down2 = self.down2(down2)
        # print("down2.shape=",down2.shape)

        #bottom
        bottom = self.maxPool3d(down2)
        # print("bottom.shape=",bottom.shape)
        bottom = self.bottom(bottom)
        # print("bottom.shape=",bottom.shape)

        #up2
        up2 = self.upsample3d(bottom) + down2
        # print("up2.shape=",up2.shape)
        up2 = self.up2(up2)
        # print("up2.shape=",up2.shape)

        #up1
        up1 = self.upsample3d(up2) + down1
