...
class CVSMNet_Downsize(nn.Module):
    def __init__(self, maxdisp):
        ...
        self.dres0 = nn.Sequential(
            nn.Conv3d(in_channels, in_channels//2, kernel_size=3, padding=1, stride=1,bias=False),
        ...
        self.dres1 = nn.Sequential(
            nn.Conv3d(in_channels//2, in_channels, kernel_size=3, padding=1, stride=(2,1,1),bias=False),
        ...
        self.dres2 = nn.Sequential(
            nn.Conv3d(in_channels, in_channels*2, kernel_size=3, padding=1, stride=(2,1,1),bias=False),
            nn.InstanceNorm3d(in_channels*2),
            Act(inplace=True),
            nn.Conv3d(in_channels*2, in_channels*2, kernel_size=3, padding=1, stride=1,bias=False),
            nn.InstanceNorm3d(in_channels*2)
        ...
        self.dres3 = nn.Sequential(
            nn.Conv3d(in_channels*2, in_channels*4, kernel_size=3, padding=1, stride=(2,1,1),bias=False),
            nn.InstanceNorm3d(in_channels*4),
            Act(inplace=True),
            nn.Conv3d(in_channels*4, in_channels*4, kernel_size=3, padding=1, stride=1,bias=False),
            nn.InstanceNorm3d(in_channels*4)
            ) 

        self.dres4 = nn.Sequential(
            nn.Conv3d(in_channels*4, in_channels*8, kernel_size=3, padding=1, stride=(2,1,1),bias=False),
            nn.InstanceNorm3d(in_channels*8),
            Act(inplace=True),
            nn.Conv3d(in_channels*8, in_channels*8, kernel_size=3, padding=1, stride=1,bias=False),
            nn.InstanceNorm3d(in_channels*8)
            )


        self.dres5 = nn.Sequential(
            nn.Conv3d(in_channels*8, in_channels*8, kernel_size=3, padding=(0,1,1), stride=1,bias=False)
            )

        self.classify = nn.Sequential(
            nn.Conv2d(in_channels*8, in_channels*8, kernel_size=3, padding=1, stride=1,bias=False),
            ...
            nn.InstanceNorm2d(in_channels*8),
            Act(inplace=True),
            nn.Conv2d(in_channels*8, 1, kernel_size=3, padding=1, stride=1,bias=False)
        )
